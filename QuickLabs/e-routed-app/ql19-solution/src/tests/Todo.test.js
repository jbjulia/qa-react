...
describe(`Todo test suite`, () => {
    ...
    describe(`Test conditional render if props.todo.todoCompleted is false`, () => {
        test(`it should render 'Edit' in the final td of the row if props.todo.todoCompleted is false`, () => {
            ...
            const links = testInstance.findByType(`span`);
    ...
    describe(`Todo event tests`, () => {

        test(`it should call selectTodo with the provided todo when the span is clicked `, () => {

            const selectTodo = jest.fn();
            const testTodo = new TodoModel();
            testTodo.todoCompleted = false;

            const testRenderer = create(<Todo todo={testTodo} selectTodo={selectTodo}/>);
            const testInstance = testRenderer.root;

            const linkSpan = testInstance.findByType(`span`);

            linkSpan.props.onClick();
